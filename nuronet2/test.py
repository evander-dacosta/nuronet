...
from nuronet2.base import MLModel, InputDetail, MLConnection, make_list
...
class AcyclicModel(MLModel):
    ...
    def __init__(self, input, output, name=None):
        ...
        for x in self.inputs:
            if(not hasattr(x, '_history')):
                cls_name = self.__class__.__name__
                raise Exception("Input tensors to {} must be Nuronet tensors".format(cls_name) + \
                                "Found an input variable with no _history and _shape metadata.")
            layer, connection_index, tensor_index = x._history
        ...
        self.internal_input_shapes = [x._shape for x in self.inputs]
        self.internal_output_shapes = [x._shape for x in self.outputs]
        ...
        def build_graph_map(tensor, seen=set(), depth=0,
            ...
            if(not layer or connection_index is None or tensor_index is None):
                layer, connection_index, tensor_index = tensor._history
        ...
        MLConnection(self,
                    ...
                    input_shapes=[x._shape for x in self.inputs],
                    output_shapes=[x._shape for x in self.outputs])
        self.is_built = True
    ...
    def run_internal_graph(self, inputs):
        ...
        for depth in depth_keys:
            ...
            for connection in connections:
                ...
                if(len(computed_data) == len(ref_input_tensors)):
                    ...
                    #Update _shape
                    if(all([hasattr(x, '_shape') for x in computed_tensors])):
                        shapes = make_list([layer.get_output_shape(x._shape) for x in computed_tensors])
        ...
        for x in self.outputs:
            ...
            if(hasattr(tensor, '_shape') and output_shapes is not None):
                shape = tensor._shape
        ...
        if(output_shapes is not None):
            input_shapes = [x._shape for x in inputs]
